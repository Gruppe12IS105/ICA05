...
func mellomLagringJSON(importEksportJSON []byte){
	err := ioutil.WriteFile("siste13mnd_IE/importEksport13mndSSB.json", importEksportJSON, 0666)
	if err != nil {
		panic(err.Error())
	}
}
...
func mellomLagringTXT(importEksportJSON []byte){
	err := ioutil.WriteFile("siste13mnd_IE/importEksport13mndSSB.txt", importEksportJSON, 0666)
	if err != nil {
		panic(err.Error())
	}
}
