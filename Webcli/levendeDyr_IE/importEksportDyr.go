...
func HentData(urlImportEksportDyr string) {
	for {
		respons, err := http.Get(urlImportEksportDyr)
		if err != nil {
			panic(err.Error())
		}
...
func mellomLagringJSON(importEksportJSON []byte) {
	err := ioutil.WriteFile("levendeDyr_IE/importEksportDyrSSB.json", importEksportJSON, 0666)
	if err != nil {
		panic(err.Error())
	}
	//os.Chmod("importEksport13mndSSB.json", 777)
}

func mellomLagringTXT(importEksportJSON []byte) {
	err := ioutil.WriteFile("levendeDyr_IE/importEksportDyrSSB.txt", importEksportJSON, 0666)
	if err != nil {
		panic(err.Error())
	}
	//os.Chmod("importEksport13mndSSB.txt", 777)
}
