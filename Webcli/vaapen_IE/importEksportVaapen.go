...
func HentData(urlImportEksportVaapen string) {
	for {
		respons, err := http.Get(urlImportEksportVaapen)
		if err != nil {
			panic(err.Error())
		}
...
func mellomLagringJSON(importEksportJSON []byte) {
	err := ioutil.WriteFile("vaapen_IE/importEksport13mndSSB.json", importEksportJSON, 0666)
	if err != nil {
		panic(err.Error())
	}
}
...
func mellomLagringTXT(importEksportJSON []byte) {
	err := ioutil.WriteFile("vaapen_IE/importEksport13mndSSB.txt", importEksportJSON, 0666)
	if err != nil {
		panic(err.Error())
	}
}
...
func convertJSON(importEksportJSON []byte) {
	...
	fmt.Println("----Import/Eksport Våpen----")
