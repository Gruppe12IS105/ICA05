...
/*
	Evig for-løkke som henter data i JSON-format, leser den inn som []bytes
	og kjører de andre funksjonene som go-routines før den sover
	i 5min. deretter den starter på nytt.
*/
...
/*
	Mellomlagrer informasjonen som txt-fil og json-fil i pakken, "generell_IE".
*/
func mellomLagringJSON(importEksportJSON []byte) {
	err := ioutil.WriteFile("generell_IE/importEksportSSB.json", importEksportJSON, 0666)
	if err != nil {
		panic(err.Error())
	}
}
...
func mellomLagringTXT(importEksportJSON []byte) {
	err := ioutil.WriteFile("generell_IE/importEksportSSB.txt", importEksportJSON, 0666)
	if err != nil {
		panic(err.Error())
	}
}
...
/*
	Konverterer data-en til human-readable ved hjelp av golang sin struct og skriver
	ut det en ønsker.
*/
func convertJSON(importEksportJSON []byte) {
	...
	fmt.Println("----Import/Eksport Generelt----")
